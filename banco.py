...
def criar_conta() -> None:
    ...
    data_nascimento: str = input('Data de nascimento do cliente (dd/mm/aaa): ')
...
def efetuar_saque() -> None:
    if len(contas) >0:
        numero: int = int(input('Infome o número da sua conta: '))

        conta: Conta = buscar_conta_por_numero(numero)

        if conta:
            valor: float = float(input('Infome o valor do saque: '))
    ...
    if len(contas) > 0:
        ...
        if conta_o:
            ...
            if conta_d:
                ...
                print(f'A conta destino com número {numero_o} não foi encontrada ')
        else:
            print(f'Sua conta com número {numero_o} não foi encontrada ')
    else:
        print('Ainda não existem contas cadastradas')
    sleep(2)
    menu()
